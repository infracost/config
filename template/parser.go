...
type Variables struct {
	...
	Branch              string
	BaseBranch          string
	DetectedProjects    []autodetect.Project
	DetectedRootModules []autodetect.RootModule
}
...
type Parser struct {
	...
	variables        Variables
	isProductionFunc func(projectName string) bool
}
...
func NewParser(repoDir string, variables Variables, isProductionFunc func(string) bool) *Parser {
	absRepoDir, _ := filepath.Abs(repoDir)
	p := Parser{repoDir: absRepoDir, variables: variables, isProductionFunc: isProductionFunc}
...
// CompileFromFile writes an Infracost config to io.Writer using the provided template path.
...
// replace returns s with all instances of old replaced by replacement.
func (p *Parser) replace(old, replacement, s string) string {
...
func (p *Parser) parseYaml(contents string) map[string]any {
	...
	_ = yaml.Unmarshal([]byte(contents), &out)
	return out
}
...
func (p *Parser) parseJSON(contents string) map[string]any {
	var out map[string]any
	_ = jsoniter.Unmarshal([]byte(contents), &out)
	return out
}

// note: template functions must have at least one return value, hence the useless bool
func (p *Parser) debug(data any) bool {
	_, _ = fmt.Fprintf(os.Stderr, "DEBUG: %#v\n", data)
	return true
}

// isProduction returns true if the project is production.
func (p *Parser) isProduction(projectName string) bool {
	if p.isProductionFunc == nil {
		return true
	}

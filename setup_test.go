...
func assertProjectMatches(t *testing.T, want, got *config.Project, prefix string) {
	...
	// Generated config now carries terraform var files under plugins.<type>.tfVarsFiles rather than
	// the deprecated terraform.var_files field. These tests express the expectation with the old
	// field for readability, so compare the effective var files from whichever representation each
	// side uses.
...
// the new plugins.<type>.tfVarsFiles blob (populated by generation and by the compat fold) or, as a
